"""
Version F — Cross-scene InfoNCE with K-subsampling + MLP patch_proj.

v1m3-E confirmed that cross-scene InfoNCE was failing in the classic
"log(K) plateau" regime: initial loss ≈ 9.x-10, no decrease past
warmup. With K ≈ 22000 and τ=0.5, the softmax is essentially uniform
and the per-negative gradient is diluted by 1/K — no signal reaches
the model.

Two fixes are stacked here:

1. `infonce_batch_subsample=1024` — restricts each minibatch to 1024
   randomly chosen rows, so K' = 1024, log(K') ≈ 6.9, and each
   negative's gradient contribution is ~20× stronger than at K=22k.

2. `patch_proj_hidden_channels=2048` — promotes the single
   `Linear(1332→1024)` to `Linear(1332→2048) → GELU → LN →
   Linear(2048→1024) → LN`. Qwen ViT's anisotropic / DC-dominated
   feature space is hard for a single linear to disentangle; an
   MLP gives the alignment head room to learn a non-trivial mapping.

`infonce_temperature` returned to 0.07 (standard CLIP) since K' is
now small enough to behave like normal contrastive learning.

All SSL / backbone / EMA / loss weights match v1m3-E.
"""
...
model = dict(
    ...
    # Standard CLIP τ. With K'=1024 (after subsample), log(K')≈6.9 and
    # the softmax behaves like normal small-K contrastive learning, so
    # no need for the τ=0.5 softening that E used at K=22k.
    infonce_temperature=0.07,
    # Random-row subsample inside the cross-scene InfoNCE. K' = 1024
    # restores per-negative gradient (was ~1/22000, becomes ~1/1024).
    infonce_batch_subsample=1024,
    # Promote patch_proj single Linear (1332→1024) to an MLP. The
    # extra non-linear capacity is needed to disentangle Qwen ViT's
    # anisotropic/DC-dominated patch embeddings.
