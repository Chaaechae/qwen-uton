"""
Version C — same architecture as B, but the enc2d alignment loss is
switched from `(1 - cos(f2, f3)).mean() * 10` to a symmetric (CLIP-style)
InfoNCE.

Why this variant exists
-----------------------
In B's runs the cosine-pull-only loss reached a trivial minimum: project
every 3D feature toward Qwen's mean patch direction. Diagnostic numbers
on a B checkpoint after many epochs:
    enc2d_loss     :  9.5 → 0.2  (looks fine)
    pos cosine     :  0.979
    neg cosine     :  0.979      ← identical → no discrimination
    R@1            :  0.004
    CKA(proj, qwen):  0.001      ← essentially random
patch_proj had effective rank 339 with one dominant direction (top σ=33,
others ~7-11): not full collapse, but every output points near the same
"Qwen mean" direction.

InfoNCE pulls the correct (point, patch) pair together AND pushes every
other pair apart. The mean-direction trivial solution becomes a high-loss
state (all off-diagonal logits are large), so patch_proj has to learn a
discriminative mapping.

Everything else (backbone, SSL losses, EMA, warm-start, loss weights)
matches v1m3-B. Only `enc2d_loss_type` and `infonce_temperature` change.
"""
