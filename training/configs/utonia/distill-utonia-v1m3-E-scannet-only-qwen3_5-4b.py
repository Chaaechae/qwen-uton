"""
Version E — Cross-scene (batch-wide) InfoNCE.

v1m3-C used per-scene InfoNCE which made K ~150-2k tractable but the
within-scene negatives turned out to be semantically too similar
(Qwen sees all indoor surfaces as a tight cluster), so the gradient
never developed: 500 iter, no decreasing trend.

v1m3-E goes the opposite direction: skip per-scene grouping, contrast
each patch against ALL patches in the batch (~10-30k). Cross-scene
negatives are easy (different rooms / objects) so the softmax has
spread → gradient flows. Trade-off: log K start ~9-10 looks high but
moves if alignment is learnable.

τ bumped from 0.07/0.03 (CLIP-style for small K) to 0.5 — softer
softmax over the much larger K keeps the gradient meaningful.

Architecture, SSL losses, EMA, warm-start, loss weights all identical
to v1m3-B / v1m3-C / v1m3-D. Only `enc2d_loss_type` and
`infonce_temperature` differ.
"""
