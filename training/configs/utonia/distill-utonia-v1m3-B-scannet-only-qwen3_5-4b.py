"""
Version B — Qwen-Utonia alignment + EMA-fixed 3D SSL regularization.

Uses `Utonia-v1m3b_qwen3_5_distill_ema` which fixes the two critical bugs in
v1m2 (teacher head init from student; per-step DINO EMA in `after_step`) so
that the mask/unmask SSL losses are providing meaningful supervision again,
on top of the Qwen 2D-3D alignment loss.

Loss weights are re-balanced vs. v1m2's `{1/8, 1/8, 2/8, 4/8}`: with Qwen
(image-text aligned VLM input features) as the 2D teacher — rather than
DINOv2 — the 2D signal carries more semantic load and the recipe leans
harder on it. The SSL terms remain as a self-distill regularizer at
lower weight: `{1/16, 1/16, 1/8, 3/4}`.
"""
