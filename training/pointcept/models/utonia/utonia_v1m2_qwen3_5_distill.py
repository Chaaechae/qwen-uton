"""
Utonia V1m2 Distill (target = Qwen3.5 vision tower)

Distillation target swapped from DINOv2 / SigLIP / RADIO to the vision tower of
Qwen Qwen3.5 unified VLM. We extract pre-merge patch tokens (hidden=1024,
patch=16, no spatial_merge) so that the distilled point feature lands in the
same manifold as the LM's input ViT features. SSL losses (mask / roll-mask /
unmask) are kept enabled together with the 2D-3D alignment loss.
"""
...
@MODELS.register_module("Utonia-v1m2_qwen3_5_distill")
class UtoniaQwen3_5Distill(PointModel):
    """
    Distillation training module that aligns Utonia (PTv3) pillar features to
    Qwen3.5 vision tower patch features (pre-merge, hidden=1024, patch=16),
    while keeping Sonata/Concerto-style SSL (mask, roll-mask, unmask) losses on.
    """
    ...
    def __init__(
        ...
        enc2d_head_num_prototypes=1024,
        teacher_custom=None,
        num_global_view=2,
        num_local_view=4,
        mask_size_start=5,
        ...
        super(UtoniaQwen3_5Distill, self).__init__()
    ...
    def load_enc2d(self, model_name, model_weight):
        # Qwen3.5 unified VLM: load full model, keep only the vision tower.
        # Weight prefix in safetensors index is `model.visual.`, so
        # `full.model.visual` is the ViT module we need.
    ...
    def ENC2D_forward(self, x):
        """
        Returns pre-merge patch features in shape (B, patch_h*patch_w, hidden).
        ...
        T = self.enc2d_model.config.temporal_patch_size  # 2
        P = self.enc2d_model.config.patch_size  # 16
        assert H_pix % P == 0 and W_pix % P == 0, (
            f"crop ({H_pix},{W_pix}) must be divisible by patch_size {P}"
        )
        h, w = H_pix // P, W_pix // P
    ...
    def forward(self, data_dict, return_point=False):
        ...
        # fire as `gloo::EnforceNotMet`). Per-rank loss values in logs are
        # acceptable.
        return result_dict
