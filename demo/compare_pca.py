...
#
# Usage:
#   # Interactive (3 Open3D windows + RGB reference):
...
#
#   # Headless / image file (matplotlib 3-D scatter):
#   python demo/compare_pca.py \
...
#
# --utonia defaults to downloading from HuggingFace `Pointcept/Utonia`.
# Provide a local utonia.pth via --utonia /path/to/utonia.pth.
...
def load_model(ckpt_path, label=""):
    """
    Load any of three checkpoint formats into a standalone PTv3:
      (a) Pointcept training-format (module.student.backbone.*)
      (b) Published Utonia HF format ({"config": ..., "state_dict": ...})
      (c) HF repo id "utonia" (or any other in utonia.MODELS).
    """
    if ckpt_path in (None, "utonia"):
        print(f"[{label}] Loading from HuggingFace: Pointcept/Utonia ...")
        custom = None if HAS_FLASH else dict(enable_flash=False)
...
def main():
    ...
    parser.add_argument(
        "--utonia", default=None,
        help="Path to utonia.pth. If omitted, downloads Pointcept/Utonia from HF.",
    )
    parser.add_argument(
        "--v1m3a", default=None,
    ...
    # Load all requested models. Skip a slot if its path wasn't provided.
    models = {}
    if args.utonia is None or os.path.isfile(args.utonia or ""):
    ...
    if not models:
        raise SystemExit("Provide at least one of --utonia / --v1m3a / --v1m3b.")
